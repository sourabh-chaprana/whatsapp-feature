...
const Layout = ({ children }) => {
  ...
  return (
    <div className="flex h-screen bg-gray-100">
      {/* Sidebar */}
      <aside
        className={`bg-[#1e2738] transition-all duration-300 ${
          collapsed ? "w-16" : "w-64"
        }`}
      >
        {/* Logo area */}
        <div className="flex items-center h-16 bg-blue-600 px-4">
          ...
          {collapsed && (
            <span className="text-white text-lg font-semibold mx-auto">WF</span>
          )}
        </div>
        ...
        <nav className="mt-4 px-2">
          ...
          >
            <i className="fas fa-cog text-lg"></i>
            {!collapsed && <span className="ml-3">Campaign</span>}
          </Link>
        </nav>
